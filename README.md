...
`serve --profile <name>`, reserved `login` and `logout` commands, and
`version [--json]`. `serve` starts the STDIO MCP server, which advertises
exactly `submit` and `await`. The Phase 1 foundation includes the validated
profile/catalog model, encrypted SQLite submissions, secure keyring
